...
class OrderItemForm(models.ModelForm):
	"""
	This form handles an ordered item, but adds a number field to modify the
		...
		if cleaned_data.get('deliver_quantity') is not None:
			if cleaned_data['deliver_quantity'] < 0:
				raise ValidationError(
					"Only a positive number of items can be delivered",
					code='invalid')
			if cleaned_data['deliver_quantity'] > self.instance.quantity - self.get_delivered(self.instance):
				raise ValidationError(
					"The number of items to deliver exceeds the ordered quantity",
					code='invalid')
		return cleaned_data
...
class DeliveryInline(admin.TabularInline):
	...
	form = DeliveryForm
	extra = 0
	fields = ['shipping_id', 'shipping_method', 'delivered_items', 'print_out',
			  'fulfilled_at', 'shipped_at']
	readonly_fields = ['delivered_items', 'print_out', 'fulfilled_at',
					   'shipped_at']
	...
	def get_fields(self, request, obj=None):
		assert obj is not None, "An Order object can not be added through " \
								"the Django-Admin"
		fields = list(super().get_fields(request, obj))
