...
class BaseItemSerializer(serializers.ModelSerializer):
	url = serializers.HyperlinkedIdentityField(lookup_field='pk',
											   view_name='shop:cart-detail')
...
class CartItemSerializer(BaseItemSerializer):
	...
	def create(self, validated_data):
		validated_data['cart'] = Cart.objects.get_or_create_from_request(
...
class BaseCartSerializer(serializers.ModelSerializer):
	...
	def represent_items(self, cart):
		raise NotImplementedError("{} must implement method `represent_items()`."
...
class CartSerializer(BaseCartSerializer):
	...
	def represent_items(self, cart):
		...
		serializer = CartItemSerializer(items, context=self.context,
										label=self.label, many=True)
		return serializer.data
...
class WatchSerializer(BaseCartSerializer):
	...
	def represent_items(self, cart):
		if self.with_items == CartItems.unsorted:
			items = CartItem.objects.filter(cart=cart, quantity=0)\
				.order_by('-updated_at')
		else:
			items = CartItem.objects.filter_watch_items(cart, self.context['request'])
		serializer = WatchItemSerializer(items, context=self.context,
										 label=self.label, many=True)
		return serializer.data
