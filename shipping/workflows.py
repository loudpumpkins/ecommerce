...
class SimpleShippingWorkflowMixin:
    """
    Workflow for simply marking the state of an Order while picking, packing and
    shipping items. It does not create a Delivery object.

    Add this class to ``settings.SHOP_ORDER_WORKFLOWS`` to mix it into the
    merchants Order model. It is mutual exclusive with
        :class:`shop.shipping.workflows.CommissionGoodsWorkflowMixin`
                [or]
        :class:`shop.shipping.workflows.PartialDeliveryWorkflowMixin`.

    It adds all the methods required for state transitions, while picking and
    packing the ordered goods for shipping.
    """
    TRANSITION_TARGETS = {
...
class CommissionGoodsWorkflowMixin(SimpleShippingWorkflowMixin):
    ...
    Add this class to ``store.order_workflow`` to mix it into the merchants
    Order model. It is mutual exclusive with
        :class:`shop.shipping.workflows.SimpleShippingWorkflowMixin`
            [or]
        :class:`shop.shipping.workflows.PartialDeliveryWorkflowMixin`.
...
class PartialDeliveryWorkflowMixin(CommissionGoodsWorkflowMixin):
    ...
    This mixin supports partial delivery, hence check that a materialized
    representation of the models :class:`shop.models.delivery.DeliveryModel` and
    :class:`shop.models.delivery.DeliveryItemModel` exists and is instantiated.

    Importing the classes :class:`shop.models.defaults.delivery.DeliveryModel`
    and :class:`shop.models.defaults.delivery_item.DeliveryItemModel` into the
    merchants ``models.py``, usually is enough. This adds all the methods
    ...
    def update_or_create_delivery(self, orderitem_data):
        ...
        # create a DeliveryItem object for each ordered item to be shipped
        # with this delivery
