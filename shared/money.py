...
# internal
from shared.util import classproperty, CURRENCIES
...
class MoneyMaker(type):
	"""
	Factory for building Decimal types, which keep track of the used currency.
	This is to avoid unintentional price allocations, when combined with
	decimals or when working in different currencies.

	# TODO - implement automatic conversion of currencies
	"""
	def __new__(cls, currency_code=None):
		def new_money(cls, value='NaN', context=None):
			...
			if isinstance(value, cls):
				assert cls._currency_code == \
					value._currency_code, "Money type currency mismatch"
			if value is None:
				value = 'NaN'
			try:
		...
		if currency_code not in CURRENCIES:
			raise TypeError("'{}' is an unknown currency code. Please check "
							"shared/util.py".format(currency_code))
		...
		attrs = {
			...
			'_currency': CURRENCIES[currency_code],
			'_cents': cents,
			'__new__': new_money  # replace AbstractClass __new__ to instantiate
		}
		# type(name, bases, dict) returns class of `bases` with the dict's members
