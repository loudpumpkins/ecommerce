...
class ManualPaymentWorkflowMixin:
	"""
	Add this class to `shop.order_workflow` to mix it into your `Order`.
	It adds all the methods required for state transitions, when used with the
	`ForwardFundPayment` provider.
	"""
	TRANSITION_TARGETS = {
	...
	def no_payment_required(self):
		"""
		Signals that an Order can proceed directly, by confirming a payment of
		value zero.
		"""
	...
	@transition(field='status', source='refund_payment',
...
class CancelOrderWorkflowMixin:
	"""
	Add this class to `settings.SHOP_ORDER_WORKFLOWS` to mix it into your `Order`.
	It adds all the methods required for state transitions, to cancel an order.
	"""
	CANCELABLE_SOURCES = {'new', 'created', 'payment_confirmed',
                          'payment_declined', 'ready_for_delivery'}
	TRANSITION_TARGETS = {
