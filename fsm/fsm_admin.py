...
class FSMAdminMixin(object):
	...
	* In the absence of specific transition permissions, the user must
	...
	fsm_field = ['state']
	change_form_template = 'fsm/change_form.html'
	...
	def response_change(self, request, obj):
		...
		if not getattr(obj, '_fsmtransition_results', None):
			return super(FSMAdminMixin, self).response_change(request, obj)
	...
	def save_model(self, request, obj, form, change):
		...
		super(FSMAdminMixin, self).save_model(request, obj, form, change)
